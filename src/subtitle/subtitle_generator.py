...
from ..models.video_data import VideoFile, AudioData
...
class SubtitleGenerator(LoggerMixin):
    ...
    def process_video_file(self,
                          ...
                          progress_callback: Optional[Callable[[str, float], None]] = None) -> Optional[SubtitleFile]:
        ...
        try:
            ...
            segments = self._perform_asr(
                audio_data,
                progress_callback=lambda p: progress_callback("asr", p) if progress_callback else None
            )
    ...
    def _extract_audio(self, video_file: VideoFile) -> Optional[AudioData]:
        ...
        try:
            ...
            # Preprocess for ASR
            preprocessed_audio = self.audio_processor.preprocess_audio_for_asr(audio_data)
            audio_data.audio_array = preprocessed_audio
            
            self.current_audio = audio_data
            self.logger.info(f"Audio extracted: {audio_data.duration:.2f}s, {audio_data.sample_rate}Hz")
    ...
    def _perform_asr(self,
        ...
        try:
            ...
            if audio_data.duration > chunk_length * 1.5:  # Use chunking for longer audio
    ...
    def get_processing_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "translation_ready": self.translation_pipeline.multi_stage_translator is not None
        }
