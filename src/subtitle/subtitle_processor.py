...
class SubtitleProcessor(LoggerMixin):
    ...
    def _clean_text(self, text: str) -> str:
        ...
        artifacts = [
            ...
            r"あー+",
            r"えー+",
            r"うー+",  # Japanese filler words
            r"um+",
            r"uh+",
            r"ah+",  # English filler words
        ]
    ...
    def _merge_short_segments(
        ...
        for next_segment in segments[1:]:
            ...
            should_merge = (
                (
                    current_duration < self.min_duration
                    or next_duration < self.min_duration
                )
                and gap <= self.merge_threshold
                and len(current_segment.text) + len(next_segment.text)
                < self.max_chars_per_line * self.max_lines_per_subtitle
            )
            ...
            if should_merge:
                ...
                current_segment = SubtitleSegment(
                    ...
                    confidence=(
                        ...
                        if current_segment.confidence and next_segment.confidence
                        else None
                    ),
    ...
    def _split_long_segments(
        ...
        for segment in segments:
            ...
            if (
                ...
                or text_length > self.max_chars_per_line * self.max_lines_per_subtitle
            ):
                # Split the segment
    ...
    def _split_segment(self, segment: SubtitleSegment) -> List[SubtitleSegment]:
        ...
        if len(sentences) > 1:
            ...
            for i, sentence in enumerate(sentences[:-1]):  # Last split is usually empty
                sentence = (
                    sentence.strip() + text[current_pos + len(sentence)]
                ...
                if sentence.strip():
                    ...
                    sub_segments.append(
                        SubtitleSegment(
                            start_time=segment.start_time
                            + current_pos / len(text) * duration,
                            end_time=segment.start_time
                            + (current_pos + len(sentence)) / len(text) * duration,
                            text=sentence.strip(),
                            confidence=segment.confidence,
                            speaker_id=segment.speaker_id,
                        )
    ...
    def analyze_segments(self, segments: List[SubtitleSegment]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "segments_text_too_long": sum(
                1
                for l in text_lengths
                if l > self.max_chars_per_line * self.max_lines_per_subtitle
            ),
        }
