...
class PreviewDialog:
    ...
    def create_widgets(self):
        ...
        self.language_combo = ttk.Combobox(
            controls_frame, textvariable=self.language_var, state="readonly", width=20
        )
        ...
        # Refresh button
        ttk.Button(controls_frame, text="🔄 Refresh", command=self.load_preview).pack(
            side=tk.RIGHT
        )
        ...
        ttk.Button(
            button_frame, text="💾 Export This View", command=self.export_current_view
    ...
    def get_translated_content(self, lang_code: str, format_type: str) -> str:
        ...
        if format_type == "SRT":
            ...
            translated_segments = []
            for i, (seg, trans) in enumerate(
                zip(self.subtitle_file.segments, translations)
            ):
        ...
        try:
            ...
            if file_path:
                ...
                tk.messagebox.showinfo(
                    "Export Complete", f"Preview exported to:\n{file_path}"
                )
