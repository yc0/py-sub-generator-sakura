...
class MainWindow(LoggerMixin):
        ...
        try:
            ...
            if current_theme != new_theme:
                try:
                    ...
                    self.logger.warning(
                        f"Theme '{new_theme}' not available, keeping '{current_theme}'"
                    )
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(
            main_frame, text="🌸 Sakura Subtitle Generator", font=("Arial", 16, "bold")
        )
    ...
    def create_file_selection_section(self, parent, row):
        ...
        info_label = ttk.Label(
            file_frame, textvariable=self.file_info_var, foreground="gray"
        )
    ...
    def create_language_selection_section(self, parent, row):
        ...
        ttk.Label(lang_frame, text="Generate subtitles for:").grid(
            row=0, column=0, sticky=tk.W
        )

        en_check = ttk.Checkbutton(
            lang_frame, text="🇺🇸 English", variable=self.lang_vars["en"]
        )
        en_check.grid(row=0, column=1, padx=10)

        zh_check = ttk.Checkbutton(
            lang_frame, text="🇹🇼 Traditional Chinese", variable=self.lang_vars["zh"]
        )
    ...
    def create_processing_controls_section(self, parent, row):
        ...
        settings_btn = ttk.Button(
            controls_frame, text="⚙️ Settings", command=self.show_settings
        )
    ...
    def create_results_section(self, parent, row):
        ...
        self.results_text = tk.Text(
            text_frame, height=8, wrap=tk.WORD, state=tk.DISABLED
        )
    ...
    def create_menu_bar(self):
        ...
        file_menu.add_command(
            label="Export Subtitles...", command=self.export_subtitles
        )
        file_menu.add_separator()
    ...
    def browse_video_file(self):
        ...
        if self.is_processing:
            messagebox.showwarning(
                "Processing", "Please wait for current processing to complete."
            )
            return
        ...
        if file_path:
            ...
            if video_file:
                video_file = self.subtitle_generator.file_handler.get_video_metadata(
                    video_file
                )
    ...
    def _show_processing_results(self, subtitle_file: SubtitleFile):
        ...
        self._add_result_text("\n🎉 Processing Results:\n")
        self._add_result_text(
            f"📊 Generated {len(subtitle_file.segments)} subtitle segments\n"
        )
    ...
    def cancel_processing(self):
        ...
        if self.is_processing and self.processing_thread:
            ...
            messagebox.showinfo(
                "Cancel", "Processing will stop after current stage completes."
            )
    ...
    def export_subtitles(self):
        ...
        try:
            ...
            exported_files = self.subtitle_generator.export_subtitles(
                self.current_subtitle_file, output_path, formats=["srt"]
            )
